...
export class GithubEntry implements Entry {
  ...
  getSourceURL(path: string, version?: string): string {
    return `https://raw.githubusercontent.com/${this.owner}/${this.repo}/${version ??
      this.defaultVersion ?? "master"}${this.path ?? ""}${path}`;
  }
  getRepositoryURL(path: string, version?: string): string {
    return `https://github.com/${this.owner}/${this.repo}/tree/${version ??
      this.defaultVersion ?? "master"}${this.path ?? ""}${path}`;
  }
  async getDirectoryListing(
    ...
    try {
      const url =
        `https://api.github.com/repos/${this.owner}/${this.repo}/contents${this
          .path ?? ""}${path}?ref=${version ?? this.defaultVersion ??
          "master"}`;
      ...
      if (res.status !== 200) {
        throw Error(
          `Got an error (${res.status}) while querying the GitHub API:\n${await res
            .text()}`,
        );
      }
